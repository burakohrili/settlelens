    ...
    <>
      ...
      <main className="mx-auto max-w-4xl py-12 px-4">
        <div className="mb-10 text-center">
          ...
          <p className="mt-3 text-[#8B7355]">{t("subtitle")}</p>
          <p className="mt-1 text-xs text-[#8B7355]">{t("disclaimerNote")}</p>
        </div>

        {posts.length === 0 ? (
          <div className="text-center py-20 text-[#8B7355]">
            <p>{t("empty")}</p>
            <Link href={`/${lang}`} className="mt-4 inline-block text-sm text-[#C8973A] hover:underline">
              ← {t("backHome")}
            </Link>
          </div>
        ) : (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <time className="text-xs text-[#8B7355]">
                    {new Date(post.publishedAt).toLocaleDateString(lang === "ar" ? "ar-SA" : lang === "tr" ? "tr-TR" : "en-US", {
                ...
                <p className="mt-2 text-sm text-[#8B7355] line-clamp-3">{post.description}</p>
        ...
        <div className="mt-12 rounded-xl bg-[#1C2B3A] p-8 text-center text-white">
          ...
          <p className="mt-2 text-sm text-gray-300">{t("ctaDesc")}</p>
          <Link
            href={`/${lang}/register`}
